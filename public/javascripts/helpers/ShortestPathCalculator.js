  ...
  .attr("class", function(d) {
    if(that.result.path !== null) {
      for (var i = 0; i < that.result.path.length; i++) {
        if ((that.result.path[i].source === d.source.name && that.result.path[i].target === d.target.name)
            || (that.result.path[i].source === d.target.name && that.result.path[i].target === d.source.name))
          return 'link bold';
      }
    }
    return 'link';
